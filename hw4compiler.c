...
void raise_error(int error_code) {
    ...
    switch (error_code) {
        ...
        case PCG_INVALID_EXPRESSION:
            printf(
                "Arithmetic equations must contain only operands, "
                "parentheses, numbers, or symbols.");
            break;
...
typedef enum {
    skipsym = 1, identsym = 2, numbersym = 3, plussym = 4, minussym = 5, multsym = 6,
...
void tokenize(int n, char * value) {
    ...
    // if (value == NULL) strcpy(lexeme[token_count].value, TOKEN_LOOKUP_TABLE[n - 4]);
    // else strcpy(lexeme[token_count].value, value);
...
int is_reserved_word(char c) {
    ...
    // Keeps going until end of program or next character is no longer alphabetic
    ...
    if (strcmp(potential_word, "odd") == 0) {
        ...
        // If the string is not reserved word, reset program index to original character
        text_index = temp;
        return 0;
    }
}

// Checks if the current character is the start of an identifier, returning 0 if not
// the ending index of the identifier if it is and -1 if it is too long
int is_ident(char c) {
...
int is_num(char c) {
    ...
    // identifier starting with a number, which is invalid, example: "45x" or "2test"
...
int is_comment(char c) {
    return c == '/' && (text_index + 1 < text_size) && text[text_index + 1] == '*';
}
...
void generate_tokens() {
    ...
    while (text_index < text_size) {
        int res;
        // Check if the current character is a special symbol (or start of special symbol)
        ...
        // Check if the current character is the start of a comment block
            ...
            text_index += 2;
            while (text_index + 1 < text_size && !(text[text_index] == '*' && text[text_index + 1] == '/')) {
                text_index++;
            }
        ...
        // If the character passes all of the previous check it is an invalid character
        else {
...
#define MAX_LEVEL 10
#define SYMBOL_LENGTH_LIMIT 12

// Structs and Enums

typedef struct {
    int kind;       // const = 1, var = 2, proc = 3
    char name[SYMBOL_LENGTH_LIMIT];  // name up to 11 chars
    int val;        // number
    int level;      // L level
    int addr;       // M address
    int mark;
...
int declared_symbol(char* symbol_name) {
    ...
    // Or if the same symbol exists in a broader scope than it is still permissible
...
void block() {
    ...
    // At the end of block, make all symbols declared in scope so they are ignored
    mark_symbols();
...
void statement() {
    ...
    if (current.type == identsym) {
        ...
        emit(IT_STO, current_level - symbol_table[symIdx].level, symbol_table[symIdx].addr);
    }
   // "call" ident
        ...
        // Double check level
        emit(IT_CAL, current_level - symbol_table[symIdx].level, symbol_table[symIdx].addr);
    ...
    // "begin" statement { ";" statement } "end"
        ...
        if (current.type != endsym) {
            ...
            return raise_error(PCG_END_EXPECTED);
        }
        next_token();
    }
    // "if" condition "then" statement
...
void factor() {
    ...
    if (current.type == identsym) {
        ...
        if (symbol_table[symIdx].kind == 1) {
            ...
            emit(IT_LOD, current_level - symbol_table[symIdx].level, symbol_table[symIdx].addr);
        }
        else {
    ...
    // "(" expression ")"
        ...
        next_token();
        expression();
        if (current.type != rparentsym) return raise_error(PCG_RIGHT_PARENTHESIS_EXPECTED);
        next_token();
    }
    else {
...
int main(int argc, char * argv[]) {
    ...
    for (int i = 0; i < code_count; i++) {
        ...
        if (code[i].op == IT_JMP || code[i].op == IT_CAL || code[i].op == IT_JPC) {
            code[i].M *= 3;
        }
